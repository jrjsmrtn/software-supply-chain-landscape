...
- **`landscape.md` gains a misconception the corpus had hit eight times without naming**: *"the
  organisation that published it is the organisation that wrote it."* CISA facilitated the SBOM types
  rather than authoring them; SSVC is CERT/CC's model with a CISA tree over it; EPSS is stewarded by
  FIRST and generated by Empirical Security; OMS is a format and `model-signing` an implementation;
  Sigstore is a project and `cosign` a client; OWASP and Ecma, the Linux Foundation and ISO/IEC; CNA
