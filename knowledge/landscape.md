...
**"The organisation that published it is the organisation that wrote it."**
Almost never, in this field. The party that *stewards* a thing and the party that *operates* or
*publishes* it are routinely different, and conflating them misattributes authority in both
directions. CISA **facilitated** the six SBOM types — that document says outright "It is not an
official US government document", and its drafting was led from the Linux Foundation and Medtronic.
SSVC is a **CERT/CC (Carnegie Mellon)** model over which CISA runs a customised tree. EPSS is
stewarded by a **FIRST** special interest group while the scores are generated by **Empirical
Security**. The same split appears without any agency involved: OMS is a format and `model-signing`
one implementation of it, Sigstore is a project and `cosign` its client, OWASP develops CycloneDX
while **Ecma** standardises it as ECMA-424, the Linux Foundation develops SPDX while **ISO/IEC**
standardises it as 5962, and within a single CVE record a CNA assigns while an ADP enriches.

The practical rule: **before citing something as authority, ask who authored it and who published
it separately.** "CISA published" and "CISA requires" are different claims; so are "the standard
says" and "the reference implementation does". Where those come apart, this corpus records both, and
the difference is usually the interesting part.
